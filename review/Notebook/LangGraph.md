...
| 特性         | `RunnableConfig`                                             | `Runtime`                                                    |
| :----------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| **主要用途** | 传递**运行实例**的配置（如线程ID、元数据），用于控制图本身的执行行为。 | 传递**业务逻辑**的上下文（如用户ID、数据库连接），用于节点内部的具体操作。 |
| **内容**     | 包含 `thread_id`、`run_id`、`tags`、`metadata` 等。          | 包含 `context`、`store`、`stream_writer`、`execution_info` 等。 |
| **典型场景** | 管理对话会话、在LangSmith中追踪运行、控制图的重入等。        | 访问用户身份、读写长期记忆、发送自定义流式数据等。           |
| **注入方式** | 直接在节点函数中声明 `RunnableConfig` 类型的参数。           | 直接在节点函数中声明 `Runtime` 类型的参数。                  |
...
由于LangGraph实现了langchain_core当中的Runnable接口，其为我们提供了`stream`和astream方法，也即流式输出方法，通过`流式输出`，就能解决前面所说到的问题。
...
| **updates**   | 图执行过程中，每一步执行后流式输出增量更新。如果在同一个步当中产生了多个增量更新，这些增量更新会分别流式输出。(`默认`) |
...
def node_1(state:MyState):
    ...
    print(f"获取了用户的输入：{user_input}")

    extra_input = interrupt(
        {"node":"node_1",
